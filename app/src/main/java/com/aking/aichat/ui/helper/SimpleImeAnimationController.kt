...
internal class SimpleImeAnimationController {
    ...
    fun startControlRequest(
        view: View, onRequestReady: ((WindowInsetsAnimationControllerCompat) -> Unit)? = null
    ) {
        ...
        isImeShownAtStart = ViewCompat.getRootWindowInsets(view)?.isVisible(WindowInsetsCompat.Type.ime()) == true
    ...
    fun insetBy(dy: Int): Int {
        val controller = insetsAnimationController ?: throw IllegalStateException(
            "Current WindowInsetsAnimationController is null." + "This should only be called if isAnimationInProgress() returns true"
        )
    ...
    fun animateToFinish(velocityY: Float? = null) {
        ...
        when {
            ...
            velocityY != null -> animateImeToVisibility(
                visible = velocityY > 0, velocityY = velocityY
            )
            // The current inset matches either the shown/hidden inset, finish() immediately
    ...
    private fun animateImeToVisibility(
        visible: Boolean, velocityY: Float? = null
    ) {
